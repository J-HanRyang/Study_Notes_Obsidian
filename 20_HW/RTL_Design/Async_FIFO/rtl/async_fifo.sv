module async_fifo #(
    ...
    always_ff @(posedge clk_wr, negedge rst_n_wr) begin
        if (!rst_n_wr) begin
            ...
            fifo_mem[wr_ptr_gray[PTR_WIDTH-1:0]] <= wr_data;
            wr_ptr_gray <= (wr_ptr_gray + 1) ^ ((wr_ptr_gray + 1) >> 1); // Convert to Gray code
        end
    end

	// Read operation
	always @(posedge clk_rd, negedge rst_n_rd) begin
		if (!rst_n_rd) begin
			rd_ptr_gray <= 0;
		end else if (rd_en && !empty) begin
			rd_data <= fifo_mem[rd_ptr_gray[PTR_WIDTH-1:0]];
			rd_ptr_gray <= (rd_ptr_gray + 1) ^ ((rd_ptr_gray + 1) >> 1); // Convert to Gray code
		end
	end
	...
	assign full  = (wr_ptr_gray == {~rd_ptr_gray_sync_1[PTR_WIDTH], rd_ptr_gray_sync_1[PTR_WIDTH-1:0]});
