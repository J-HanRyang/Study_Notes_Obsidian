...
- Full 판별: rd_ptr_gray_sync가 실제보다 오래됐다면 실제로는 공간이 있는데 full로 판단할 수 있다. 데이터 유실 없이 쓰기를 잠깐 막는 것이므로 안전하다.
- Empty 판별: wr_ptr_gray_sync가 오래됐다면 실제로는 데이터가 있는데 empty로 판단할 수 있다. 읽기를 잠깐 막는 것이므로 안전하다.
...
- **gray = binary ^ (binary >> 1)**:
	- 표준 바이너리→그레이 변환 공식. 다음 포인터 값에 바로 적용해서 1사이클 안에 갱신.
